...
$(document).contextmenu(function (ev) {
    $kuang.on('mousedown', function (ev) {
        return false;
    })
    $kuang.css({
        'width': '0',
    ...
    $yj_list.show().css({
        'top': `${ev.clientY - 128}px`,
        'left': `${ev.clientX - 304}px`
    });
    // ev.preventDefault();
    return false; // 阻止鼠标右键默认行为
});
$yj_list.on('click', function (ev) {
